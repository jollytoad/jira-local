...
- `deno task jira-local <command>` is the general form (`pull`/`categorize`);
...
- `src/cli.ts` builds the cliffy `Command` (`jira-local` with `pull` and
  `categorize` subcommands); each command lives in `src/commands/<name>.ts` with
